...
def load_model_and_may_interpolate(ckpt_path, model, model_key, model_prefix):
    ...
    for pos_embed_key in ("vision_pos_embed", "pos_embed", "mtpnet.encoder.embed_positions.A.weight"):
        if pos_embed_key in checkpoint_model:
            pos_embed_checkpoint = checkpoint_model[pos_embed_key]
            embedding_size = pos_embed_checkpoint.shape[-1]
            if pos_embed_key == "mtpnet.encoder.embed_positions.A.weight":
                # being consistent with Fairseq, which starts from 2 for position embedding
                torchscale_model = True
                num_patches = model.mtpnet.vision_embed.num_patches
                num_extra_tokens = model.mtpnet.vision_embed.num_position_embeddings() + 2 - num_patches
            else:
