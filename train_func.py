...
# from datasets import get_sentencepiece_model_for_mtpnet
...
def get_handler(args):
    if args.task == "vqav2":
        ...
        raise NotImplementedError("Oops, %s is not support." % args.task)
...
def train_one_epoch(
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if not math.isfinite(loss_value):
            print("Loss: {}, training stopped.".format(loss_value))
            sys.exit(1)
